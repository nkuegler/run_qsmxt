...
#
# Usage: ./call_synthstrip_n.sh [--acqs <ACQ_TYPES>] [--no-csf] <INPUT_DIR> <OUTPUT_DIR> <SUBJECT1> <SUBJECT2> ...
#
# Options:
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -*)
            ...
            echo "Usage: $0 [--acqs <ACQ_TYPES>] [--no-csf] <INPUT_DIR> <OUTPUT_DIR> <SUBJECT1> [SUBJECT2] ..."
            echo "Options:"
...
SLURM_SCRIPT="/data/u_kuegler_software/git/qsm/run_qsmxt/synthstrip_slurm_n.sh"
...
for subj in "$@"; do
    ...
    while IFS= read -r -d '' session_dir; do
        if [ -d "$session_dir" ]; then
            ...
            if [ -d "$anat_dir" ] && [ -n "$(find "$anat_dir" -name "*.nii" -o -name "*.nii.gz" 2>/dev/null)" ]; then
                ...
                jobid=$(sbatch -p ${SLURM_PARTITIONS} ${SLURM_SCRIPT} "$INPUT_DIR" "$OUTPUT_DIR" "$subj" "$session" "$ACQ_TYPES" "$NO_CSF" | awk '{print $4}')
    ...
    if [ "$sessions_found" = false ]; then
        ...
        if [ -d "$anat_dir" ] && [ -n "$(find "$anat_dir" -name "*.nii" -o -name "*.nii.gz" 2>/dev/null)" ]; then
            ...
            jobid=$(sbatch -p ${SLURM_PARTITIONS} ${SLURM_SCRIPT} "$INPUT_DIR" "$OUTPUT_DIR" "$subj" "" "$ACQ_TYPES" "$NO_CSF" | awk '{print $4}')
